// functions/fix-stats-v2.js
// CORRECTED one-time admin script — run once, then delete this file.
// The previous version patched the outer KV wrapper. The real data your
// frontend reads is double-encoded: KV value -> {content:[{type:"text",
// text:"<escaped JSON string>"}]} -> that string, parsed again, is the
// actual {sector, headline, stats, achievements...} object. This version
// reaches into that nested string, patches `stats` there, and re-encodes
// it back into the same shape.
//
// Run: GET /fix-stats-v2?key=RUNONCE2

const SECRET = "RUNONCE2";
...
export async function onRequestGet(context) {
  ...
  for (const [cacheKey, stats] of Object.entries(STATS_BY_SECTOR)) {
    ...
    try {
      ...
      if (outer && outer.content && outer.content[0] && typeof outer.content[0].text === "string") {
        const inner = JSON.parse(outer.content[0].text);
        inner.stats = stats;
        outer.content[0].text = JSON.stringify(inner);
        if ("stats" in outer) delete outer.stats;
        await kv.put(cacheKey, JSON.stringify(outer));
        results.push({ cacheKey, status: "UPDATED (nested content.text)", statCount: stats.length });
      } else if (outer && typeof outer.sector !== "undefined") {
        outer.stats = stats;
        await kv.put(cacheKey, JSON.stringify(outer));
        results.push({ cacheKey, status: "UPDATED (flat object)", statCount: stats.length });
      } else {
        results.push({ cacheKey, status: "UNRECOGNISED SHAPE - not patched", sample: Object.keys(outer || {}) });
      }
    } catch (e) {
